...
export default function Admin() {
  ...
  const salvarProduto = async () => {
    ...
    if (editandoProduto) {
      const { error } = await supabase.from('produtos').update(dadosProduto).eq('id', editandoProduto);
      if (error) mostrarMensagem('❌ Erro ao atualizar produto!', 'erro');
      else mostrarMensagem('✅ Produto atualizado com sucesso!');
    } else {
      const { error } = await supabase.from('produtos').insert([dadosProduto]);
      if (error) mostrarMensagem('❌ Erro ao adicionar produto!', 'erro');
      else mostrarMensagem('✅ Produto adicionado com sucesso!');
    }
  ...
  const salvarKit = async () => {
    ...
    if (editandoKit) {
      ...
      if (error) { mostrarMensagem('❌ Erro ao atualizar kit!', 'erro'); setSalvando(false); return; }
      ...
      if (error) { mostrarMensagem('❌ Erro ao criar kit!', 'erro'); setSalvando(false); return; }
      kitId = data?.[0]?.id;
    }
  ...
  const toggleProdutoKit = (id) => {
    setProdutosDoKit(prev => prev.includes(id) ? prev.filter(p => p !== id) : [...prev, id]);
  };
              ...
              <section style={styles.card}>
                ...
                <div style={styles.checks}>
                  <label style={styles.checkLabel}>
                    <input type="checkbox" checked={formProduto.destaque} onChange={e => setFormProduto({ ...formProduto, destaque: e.target.checked })} />
                    ⭐ Destaque na Home
                  </label>
                  <label style={styles.checkLabel}>
                    <input type="checkbox" checked={formProduto.ativo} onChange={e => setFormProduto({ ...formProduto, ativo: e.target.checked })} />
                    ✅ Ativo
                  </label>
                </div>
              ...
              <section style={styles.card}>
                <div style={styles.listaHeader}>
                  ...
                  <div style={styles.buscaWrapper}>
                    <span>🔍</span>
                    <input style={styles.buscaInput} placeholder="Buscar produto..." value={buscaProduto} onChange={e => setBuscaProduto(e.target.value)} />
                      ...
                      <div key={p.id} style={{ ...styles.item, opacity: p.ativo ? 1 : 0.5 }}>
                        ...
                        <div style={styles.itemInfo}>
                          ...
                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                            ...
                            <span style={styles.tag}>{p.categoria}</span>
            ...
            <>
              <section style={styles.card}>
                ...
                <div style={{ marginTop: 24 }}>
                  ...
                  <div style={{ ...styles.buscaWrapper, margin: '10px 0' }}>
                    <span>🔍</span>
                    <input style={styles.buscaInput} placeholder="Buscar produto para o kit..." value={buscaProdutoKit} onChange={e => setBuscaProdutoKit(e.target.value)} />
                ...
                <div style={styles.checks}>
                  <label style={styles.checkLabel}>
                    <input type="checkbox" checked={formKit.ativo} onChange={e => setFormKit({ ...formKit, ativo: e.target.checked })} />
                    ✅ Kit ativo
                  </label>
                </div>
                <div style={styles.botoes}>
              ...
              <section style={styles.card}>
                <div style={styles.listaHeader}>
                  ...
                  <div style={styles.buscaWrapper}>
                    <span>🔍</span>
                    <input style={styles.buscaInput} placeholder="Buscar kit..." value={buscaKit} onChange={e => setBuscaKit(e.target.value)} />
